...
具体来讲，**第一，纷享销客把通用型 Paas 平台的颗粒度做的足够细，** 能够更好的支持企业自建业务体系。以客户管理页的布局自定义为例，配置组件种类丰富，支持通用型、业务型（如发起业务流程管理）、字段型等等；支持配置多种业务关联、**还可以对所关联的页面，比如客户详情页中的表单字段，如名称、公司等进行修改。** 能够支持全面、多样的业务个性化需求（如下图：纷享销客pass配置界面），

![](assets/image3.png)

相比之下，友商销售易的 Paas 平台可配置化程度没有纷享销客的高（图 2、图 3）。
